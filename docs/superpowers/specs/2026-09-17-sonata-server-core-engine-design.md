...
## 14. Amendments (2026-09-18, after implementation + whole-branch review)

- **§6 `/healthz`:** the body lists only *configured* networks (`{ db, networks: { testnet: 'ok' | 'error', … } }`); an unconfigured network is omitted, not `'disabled'`. The route returns **503 only when the DB ping fails**; a degraded RPC is reported in the body with 200 (liveness must not depend on third-party RPC).
- **§6 `/submit` and `GET /tx/:hash`:** the status carries `return_value?` (base64 ScVal of the contract return, SUCCESS only) and `result_xdr?` (base64 `TransactionResult`, SUCCESS and FAILED) — two fields, never one overloaded `result`. `DUPLICATE` / `TRY_AGAIN_LATER` from `sendTransaction` are treated as `pending` (with hash) and polled; `sendTransaction` is never retried.
- **§6 `/tx` `fee`:** the request `fee` is the per-operation inclusion fee (stroops, default 100); the response `fee` is the assembled total read from the built XDR.
- **§6 `source`:** validated as an Ed25519 public key (`G…`) on `/call` (optional) and `/tx` (required) and in the MCP `call_*`/`build_*` tools → `400 invalid_args` with `details.path: 'source'`.
- **§6.2 additions:** `route_not_found` 404 (unknown route), `rate_limited` 429, `submit_rejected` 422 (`sendTransaction` → `ERROR`, `details.errorResult`), `contract_not_found` 404 also from the pipeline's fetch step when the contract does not exist on chain. Zod body errors carry `details.path` and `details.issues`.
- **§6.1:** the encoding conventions are documented in llms.txt's conventions line and in per-field OpenAPI `description`s (not as a table in `info.description`). Enums are integers, unions `"Name"` / `{ tag, values }`, tuples use JSON Schema 2020-12 `prefixItems`, `Option` is the value or `null`.
- **§6.3:** Fastify runs with `trustProxy: true` so rate limiting keys on the client IP behind Fly's proxy. Known follow-up: key on `Fly-Client-IP` (or trust only Fly's proxy CIDRs) — with `trustProxy: true` the left-most `X-Forwarded-For` entry is client-controlled.
- **§7:** supersedes "one `McpServer` per contract, cached in-process": a fresh `McpServer` + `NodeStreamableHTTPServerTransport` is built per request from the cached `ContractModel`. Tool input flattens the function's arguments and adds `source`; a contract argument literally named `source` therefore collides (known limitation; reserved-name escape is future work). `search_functions` matches name **or** doc.
- **§3 cache:** the registry cache is a bounded LRU (default 256 entries, constructor option `cacheMax`).
- **§3.2 hints:** the name-based read list is broader than the spec's sketch (`admin`, `owner`, `version`, `spec`; prefixes `total_`, `list_`, `read_`, `view_`; suffixes `_at`, `_for`) — see `server/src/spec/hints.ts`. The hint gates nothing security-relevant (`build_*` is gated by `mcp_scope`).
- **Renaming** (`PATCH name`, or re-registering with a new `name` and an unchanged wasm hash) regenerates llms.txt and OpenAPI.
- **Testnet fixture:** the kitchen-sink contract (now 16 functions incl. `echo_nested`) is deployed on testnet as `CADY5JYDD7VE7M42HLGJILZAYRPJXPA7C7AOOJOLA44ECZDOA4NVULTP` (deployed before `echo_nested` was added; the e2e flow does not depend on it).
