...
- **SAC contracts**: stellar-sdk 17's `getContractWasmByContractId` already rejects a SAC with an `Error` whose message contains "is a Stellar Asset Contract (SAC), which has no Wasm bytecode" (verified on testnet with the native XLM SAC `CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC`). `RpcChain.getContractWasm` maps that message to `ChainError(400, 'sac_unsupported', 'contract <id> is a Stellar Asset Contract (SAC) — it has no WASM spec; SAC support arrives in a later release')` instead of the generic 502. The pipeline records it as the `fetch` step's error and status `failed`; the site's pipeline view already shows step errors.
...
- `lib/session.js` (storage: `getSession/setSession/clearSession/onSessionChange`, expired sessions dropped on read) and `lib/auth.js`: `signIn()` = `connect()` → `POST /auth/challenge` → `signTransaction` → `POST /auth/token` → persist `{ token, address, expires_at }` in `localStorage['sonata.session']`; `signOut()` clears it and calls `wallet.disconnect()`; `getSession()`; expired sessions are dropped on read.
