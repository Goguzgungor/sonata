...
export const docsRoutes = (deps: Deps): FastifyPluginAsync => async (app) => {
  ...
  app.get<{ Params: { id: string }; Querystring: Record<string, unknown> }>('/c/:id/events', async (req, reply) => {
    const q = req.query;
    if (q.format === 'csv') {
      const body = await deps.history.csv(req.params.id, q);
      return reply.type('text/csv; charset=utf-8').header('content-disposition', `attachment; filename="${req.params.id}-events.csv"`).send(body);
    }
