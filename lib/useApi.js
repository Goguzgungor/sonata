...
const MAX_POLL_DELAY = 15_000;

/**
 * Re-runs `fetcher` every `every` ms until `until(data)` is true (or `enabled` is false).
 * Consecutive failures double the interval (capped at 15 s) so a down API isn't hammered; a success resets it.
 */
export function usePoll(fetcher, { every = 1500, until, enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    let stopped = false; let timer = null; let delay = every;
    const tick = async () => {
      try {
        ...
        delay = Math.min(delay * 2, MAX_POLL_DELAY);
      }
